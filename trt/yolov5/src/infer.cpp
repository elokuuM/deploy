...
int inferEngine(char* ege_file_path, string input_path) {
    ...
    for (int i = 0; i < file_names.size(); i++){
        ...
        cout<<"["<<i+1<<"/"<<file_names.size()<<"] "<<"./pred/" + Getfilename(input_path, file_names[i])<<endl;
...
void yolopostprocess(float* netout, std::vector<detect_result> &results) {
    ...
    cv::Mat det_output(25200, 85, CV_32FC1);
    memcpy((uchar*)det_output.data, netout, 25200*85* sizeof(float));
