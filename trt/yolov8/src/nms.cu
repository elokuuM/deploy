...
__global__ void decode_kernel(float* predict, int num_bboxes, int num_classes, float confidence_threshold, float* parray, int max_objects){  
    ...
    // if(objectness < confidence_threshold) #yolov8 anchor free
    //     return;
...
void decode_kernel_invoker(float* predict, int num_bboxes, int num_classes, float confidence_threshold, float nms_threshold, float* parray, int max_objects){
    ...
    decode_kernel<<<grid, block>>>(predict, num_bboxes, num_classes, confidence_threshold, parray, max_objects);

    grid = grid_dims(max_objects);
    block = block_dims(max_objects);
    fast_nms_kernel<<<grid, block>>>(parray, max_objects, nms_threshold);
}
